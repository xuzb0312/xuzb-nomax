...
public class CheckboxListTag extends Tag{
	private static final long serialVersionUID = -230008042501286758L;
	// 属性
	...
	private boolean primary;// 风格是否为原始风格

	// 事件
	private String onCheck;// 选择(checked,value,elem,othis)
	...
	public int doEndTag() throws JspException {
		try {
			...
			String[] valueArr = this.value.split(",");
			HashMap<String, String> mapValue = new HashMap<String, String>();
			for (int i = 0, n = valueArr.length; i < n; i++) {
				mapValue.put(valueArr[i], null);
			}
			...
			// 具体内容的渲染--begin
			strBF.append("<div obj_type=\"checkboxlist\" id=\"" + this.name
					+ "\">");
			this.renderOptions(strBF, dsCode, mapValue, formTag);
			if (mapValue.size() > 0) {// 还有些有值，但是没有项目的也要进行渲染
				DataSet dsCodeOther = new DataSet();
				Object[] arrKeys = mapValue.keySet().toArray();
				for (int i = 0, n = arrKeys.length; i < n; i++) {
					...
					dsCodeOther.addRow();
					dsCodeOther.put(dsCodeOther.size() - 1, "code", key);
					dsCodeOther.put(dsCodeOther.size() - 1, "content", key);
				}
				this.renderOptions(strBF, dsCodeOther, mapValue, formTag);
			}
			strBF.append("</div>");
			// --end
			strBF.append("</div>");
			...
			strBF.append("function ")
				...
				.append("_chk_opts_" + this.name + "Func" + this.randomIndex
						+ "(checked,value,elem,othis){");
			strBF.append("getObject(\"").append(formTag.getName() + "."
					+ this.name).append("\").chkValue(true);");
			if (!StringUtil.chkStrNull(this.onCheck)) {
				strBF.append(this.onCheck + "(checked,value,elem,othis);");
			}
			strBF.append("}");
			strBF.append("</script>");
	...
	private void renderOptions(StringBuffer strBF, DataSet dsCode,
			HashMap<String, String> mapValue, FormTag formTag) throws AppException {
		for (int i = 0, n = dsCode.size(); i < n; i++) {
			String code = dsCode.getString(i, "code");
			String content = dsCode.getString(i, "content");
			strBF.append("<input type=\"checkbox\" id=\"chk_opts_")
				.append(this.name)
				.append("_")
				.append(code)
				.append("\" lay-filter=\"")
			...
			if (this.primary) {
				strBF.append(" lay-skin=\"primary\"");
			}
			strBF.append("/>");
			// 附加事件
			formTag.addEvent("checkbox", formTag.getName() + "_chk_opts_"
					+ this.name + "_" + code, formTag.getName() + "_chk_opts_"
