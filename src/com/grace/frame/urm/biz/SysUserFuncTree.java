 ...
 * @author yjc
 */
public class SysUserFuncTree extends TreeBuilder{
	private HashMap<String, String> userleafMap = new HashMap<String, String>();
	...
	public void entry(DataMap para) throws Exception {
		...
		for (int i = 0, n = dsLeafGn.size(); i < n; i++) {
			this.userleafMap.put(dsLeafGn.getString(i, "gnid"), null);
		}
		...
		for (int i = 0, n = dsYwlx.size(); i < n; i++) {
			...
			pdm.put("fgn", gnid);
			this.addNode("createSubNode", pdm, gnid, text, true, null, this.userleafMap.containsKey(gnid), null);
		}
	}
	...
	public void createSubNode(DataMap para) throws Exception {
		...
		for (int i = 0, n = dsYwlx.size(); i < n; i++) {
			...
			DataMap pdm = new DataMap();
			pdm.put("fgn", gnid);
			this.addNode("createSubNode", pdm, gnid, text, false, null, this.userleafMap.containsKey(gnid), null);
		}
	}
}
