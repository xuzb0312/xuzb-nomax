 ...
 * 
 * jQuery EasyUI tabs Extensions 1.0 beta jQuery EasyUI tabs 组件扩展
 * jeasyui.extensions.tabs.js 二次开发 流云 最近更新：2014-04-09
 * 
 ...
 * 
 * 
 * @since 2015年7月2日--modify.yjc.修改部分内容
 */
(function ($, undefined) {
    $.fn.tabs.extensions = {};
    ...
    var onContextMenu = function (e, title, index) {
        ...
        if (opts.enableConextMenu) {
            ...
            var panel = t.tabs("getTab", index),
                ...
                m0 = {
                    text: '在新页面中打开', iconCls: 'icon-shape-move-forwards', disabled: panelOpts.href && panelOpts.iniframe ? false : true,
                    handler: function () { t.tabs("jumpTab", index); }
                },
                m1 = {
                    text: "显示 Option", iconCls: "icon-application-form", disabled: opts.showOption ? false : true, children: [
                        { text: "选项卡组 Option", iconCls: "icon-tab-go", handler: function () { t.tabs("showOption"); } },
                        { text: "该选项卡 Option", iconCls: "icon-tab", handler: function () { t.tabs("showOption", index); } }
                    ]
                },
                m2 = {
                    text: "关闭选项卡", iconCls: "icon-application-form-delete", disabled: panelOpts.closable ? false : true,
                    handler: function () { t.tabs("closeClosable", index); }
                },
                m3 = {
                    text: "关闭其他选项卡", iconCls: "icon-cancel", disabled: otherTabs.length ? false : true,
                    handler: function () { t.tabs("closeOtherClosable", index); }
                },
                m4 = {
                    text: "刷新选项卡", iconCls: "icon-table-refresh", disabled: panelOpts.refreshable && panelOpts.href ? false : true,
                    handler: function () { t.tabs("refresh", index); }
                },
                m5 = {
                    text: "关闭左侧选项卡", iconCls: "icon-tab-close-left", disabled: leftTabs.length ? false : true,
                    handler: function () { t.tabs("closeLeftClosable", index); }
                },
                m6 = {
                    text: "关闭右侧选项卡", iconCls: "icon-tab-close-right", disabled: rightTabs.length ? false : true,
                    handler: function () { t.tabs("closeRightClosable", index); }
                },
                m7 = {
                    text: "关闭所有选项卡", iconCls: "icon-cross", disabled: allTabs.length ? false : true,
                    handler: function () { t.tabs("closeAllClosable"); }
                },
                m8 = {
                    text: "新建选项卡", iconCls: "icon-tab-add", disabled: opts.enableNewTabMenu ? false : true,
                    handler: function () { t.tabs("newTab", index); }
                },
                m9 = {
                    text: "重复选项卡", iconCls: "icon-control-repeat", disabled: panelOpts.repeatable ? false : true,
    ...
    var panelOptions = $.fn.tabs.extensions.panelOptions = $.extend({}, $.fn.panel.defaults, {
        ...
        // 该选项卡是否具有刷新功能。
        refreshable: false,//mod.yjc.2015年7月2日 不支持刷新，因为刷新会出错。
    ...
    var methods = $.fn.tabs.extensions.methods = {
        ...
        // which: 可选值参数；表示需要在新页面打开的的选项卡的 索引号(index)
		// 或者原标题名(title)；如果未传入该参数，则对当前选中的选项卡进行操作。
        // 返回值：返回当前选项卡控件 easyui-tabs 的 jQuery 对象。
    ...
    var defaults = $.fn.tabs.extensions.defaults = {
        ...
        // "none": 表示不显示遮蔽层和进度条
        updateProgress: "none"// mod.yjc.2015年7月2日 修改为none;
    };
    ...
    $.fn.extend({
        ...
        // 扩展 jQuery 对象的实例方法；用于刷新当前对象所在的 easyui-tabs 当前选项卡(支持当前选项卡页面为 iframe
		// 加载的情况)。
        // 该方法定义如下参数：
