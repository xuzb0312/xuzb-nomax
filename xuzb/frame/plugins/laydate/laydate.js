...
!function(win) {
	...
	Dates.getPath = (function() {
		...
		return config.path ? config.path : jsPath.substring(0, jsPath
				.lastIndexOf("/") + 1);
	}());
	...
	Dates.query = function(node) {
		...
		var node = (Dates.trim(node)).split(' '), elemId = doc[byid](node[0]
				.substr(1)), arr;
		if (!elemId) {
			return;
		} else if (!node[1]) {
			...
			var find, child = node[1].substr(1), exp = new RegExp(
					'\\b' + child + '\\b');
			arr = []
			find = doc.getElementsByClassName ? elemId
					.getElementsByClassName(child) : elemId[tags]('*');
	...
	Dates.run = function(options) {
		...
		if (even && target.tagName) {
			...
			Dates.each((elem.length | 0) > 0 ? elem : [ elem ], function(ii,
					that) {
	...
	Dates.timeVoid = function(times, index) {
		...
		} else if (Dates.ymd[1] + 1 == Dates.maxs[1]
				&& Dates.ymd[2] == Dates.maxs[2]) {
	...
	Dates.check = function() {
		var reg = Dates.options.format
				.replace(/yyyy|MM|dd|hh|mm|ss/g, '\\d+\\').replace(/\\$/g, '');
		var exp = new RegExp(reg), value = Dates.elem[as.elemv];
		var arr = value.match(/\d+/g) || [], isvoid = Dates.checkVoid(arr[0],
				arr[1], arr[2]);
	...
	Dates.viewDate = function(Y, M, D) {
				...
				function(i, elem) {
					...
					if (i < log.FDay) {
						...
						MM = MM === 1 ? 12 : MM - 1;
							...
							&& i < log.FDay + Dates.months[log.ymd[1]]) {
					...
					Dates.options.festival
							&& Dates.festival(elem, MM + '.' + DD);
		...
		Dates.each(as.mms, function(i, elem) {
			var getCheck = Dates.checkVoid(Dates.ymd[0], (elem
					.getAttribute('m') | 0) + 1);
		...
		// 定位时分秒
		log.times = [ Dates.inymd[3] | 0 || 0, Dates.inymd[4] | 0 || 0,
				Dates.inymd[5] | 0 || 0 ];
		Dates.each(new Array(3),
				function(i) {
					Dates.hmsin[i].value = Dates.digit(Dates.timeVoid(
							log.times[i], i) ? Dates.mins[i + 3] | 0
							: log.times[i] | 0);
				});
		...
		Dates
				.each(
						...
						function(i) {
							if (i === 7) {
								str += '<li '
										+ (parseInt(as.year.value) === YY ? 'class="' + as[2] + '"'
												: '') + ' y="' + YY + '">' + YY
										+ '年</li>';
							} else {
								str += '<li y="' + (YY - 7 + i) + '">'
		...
		Dates.each(S('#laydate_ys li'), function(i, elem) {
			if (Dates.checkVoid(elem.getAttribute('y'))[0] === 'y') {
				...
				Dates.on(elem, 'click', function(ev) {
					...
					Dates.viewDate(this.getAttribute('y') | 0, Dates.ymd[1],
							Dates.ymd[2]);
				});
			}
		});
	...
	Dates.iswrite = function() {
		...
		Dates.shde(as.oclear,
				!('isclear' in Dates.options ? Dates.options.isclear : 1));
		Dates.shde(as.otoday,
				!('istoday' in Dates.options ? Dates.options.istoday : 1));
		Dates.shde(as.ok, !('issure' in Dates.options ? Dates.options.issure
				: 1));
	};
	...
	Dates.view = function(elem, options) {
		...
		Dates.mm = log.mm = [ Dates.options.min || config.min,
				Dates.options.max || config.max ];
		Dates.mins = log.mm[0].match(/\d+/g);
		Dates.maxs = log.mm[1].match(/\d+/g);

		as.elemv = /textarea|input/
				.test(Dates.elem.tagName.toLocaleLowerCase()) ? 'value'
				: 'innerHTML';
		...
		if (!S('#' + as[0])) {
			...
			div.innerHTML = log.html = '<div class="laydate_top">'
					...
					+ '<a class="laydate_choose laydate_chnext laydate_tab"><cite></cite></a>'
					+ '<div class="laydate_yms" id="laydate_ms">'
					+ function() {
						var str = '';
						Dates.each(new Array(12), function(i) {
							str += '<span m="' + i + '">' + Dates.digit(i + 1)
									+ '月</span>';
						});
						return str;
					}()
		...
		options.zIndex ? Dates.box.style.zIndex = options.zIndex : Dates
				.removeCssAttr(Dates.box, 'z-index');
	...
	Dates.reshow = function() {
		Dates.each(Dates.query('#' + as[0] + ' .laydate_show'), function(i,
				elem) {
	...
	Dates.parse = function(ymd, hms, format) {
		ymd = ymd.concat(hms);
		format = format
				|| (Dates.options ? Dates.options.format : config.format);
	...
	Dates.creation = function(ymd, hide) {
		...
		var getDates = Dates.parse(ymd, [ hms[0].value, hms[1].value,
				hms[2].value ]);
		Dates.elem[as.elemv] = getDates;
		if (!hide) {
			Dates.close();
			typeof Dates.options.choose === 'function'
					&& Dates.options.choose(getDates);
		}
	};
	...
	Dates.events = function() {
		...
		Dates.each(S('#laydate_ms span'), function(i, elem) {
			Dates.on(elem, 'click', function(ev) {
				...
				if (!Dates.hasClass(this, as[1])) {
					Dates.viewDate(Dates.ymd[0], this.getAttribute('m') | 0,
							Dates.ymd[2]);
				}
			});
		});
		...
		Dates.each(S('#laydate_table td'), function(i, elem) {
			Dates.on(elem, 'click', function(ev) {
				if (!Dates.hasClass(this, as[1])) {
					Dates.stopmp(ev);
					Dates.creation( [ this.getAttribute('y') | 0,
							this.getAttribute('m') | 0,
							this.getAttribute('d') | 0 ]);
				}
			});
		});
		...
		Dates.on(as.otoday, 'click', function() {
			var now = new Date();
			Dates.creation( [ now.getFullYear(), now.getMonth() + 1,
					now.getDate() ]);
		});
		...
		Dates.on(as.ok, 'click', function() {
			if (Dates.valid) {
				Dates
						.creation( [ Dates.ymd[0], Dates.ymd[1] + 1,
								Dates.ymd[2] ]);
			}
		});
		...
		Dates.msg = function(i, title) {
			...
			if (typeof i === 'string') {
				str += '<p>' + i + '</p>';
				Dates.shde(S('#' + as[0]));
				Dates.removeClass(log.times, 'laydate_time1').addClass(
						log.times, 'laydate_msg');
			} else {
				if (!log.hmsarr[i]) {
				...
				Dates[i === 0 ? 'removeClass' : 'addClass'](log.times,
						'laydate_time1');
			}
		...
		Dates.on(doc, 'mouseup', function() {
			...
			if (box && box.style.display !== 'none') {
				...
				if (codes === 13 && Dates.elem) {
					Dates.creation( [ Dates.ymd[0], Dates.ymd[1] + 1,
							Dates.ymd[2] ]);
				}
			});
	};
